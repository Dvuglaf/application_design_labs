...
#define BENCODE_MAP_PROXY_RELOP(op)                                           \
  ...
  using data = basic_data<std::variant, unsigned long long, std::string, std::vector,
                          map_proxy>;
  using data_view = basic_data<std::variant, unsigned long long, std::string_view,
                               std::vector, map_proxy>;

#ifdef BENCODE_HAS_BOOST
  using boost_data = basic_data<boost::variant, unsigned long long, std::string,
                                std::vector, map_proxy>;
  using boost_data_view = basic_data<boost::variant, unsigned long long,
