...
T torrent_file::get(std::string key) const {
    ...
    return std::get<T>(dict->find(key).operator*().second);
}
...
std::vector<std::string> torrent_file::split_piece_hashes() const {
    const auto pieces = get<bencode::string>("pieces");
    if (pieces.empty())
...
unsigned long long torrent_file::get_file_size() const {
    const auto file_size = get<bencode::integer>("length");
    if (!file_size)
...
unsigned long long torrent_file::get_piece_length() const {
    const auto piece_length = get<bencode::integer>("piece length");
    if (!piece_length)
